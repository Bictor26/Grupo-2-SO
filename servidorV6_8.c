...
//aceptado=0 neutro
void Inicializar (TablaPartidas tabla, int i) //Inicializa el campo aceptado y socket a 0 de todos los jugadores
{
...
void ListadoRivales (TablaPartidas tabla, char nombre[20], char listado[200])	//Lista los jugadores, separados por comas, que han jugado con el usuario con el nombre introducido
{
	strcpy(listado, "1/");
	...
	for (int id=0;id<100;id++){
		...
		if ((tabla[id].aceptado1 == 1 || tabla[id].aceptado1 == NULL) &&
			...
			if (tabla[id].aceptado1 == 1 && strcmp(tabla[id].nombre1, nombre)==0)
			{
				if (tabla[id].aceptado4 == 1)
				{
					sprintf(listado, "%s%s, %s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre2, tabla[id].nombre4);
				}
				else if (tabla[id].aceptado3 == 1)
				{
					sprintf(listado, "%s%s, %s, ", listado, tabla[id].nombre2, tabla[id].nombre3);
				}
				else if (tabla[id].aceptado2 == 1)
				{
					sprintf(listado, "%s%s, ", listado, tabla[id].nombre2);
				}
				
			}
			else if (tabla[id].aceptado2 == 1 && strcmp(tabla[id].nombre2, nombre)==0)
			{
				if (tabla[id].aceptado4 == 1)
				{
					sprintf(listado, "%s%s, %s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre3, tabla[id].nombre4);
				}
				
				else if (tabla[id].aceptado3 == 1)
				{
					sprintf(listado, "%s%s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre3);
				}
				else if (tabla[id].aceptado1 == 1)
				{
					sprintf(listado, "%s%s, ", listado, tabla[id].nombre1);
				}
				
			}
			else if (tabla[id].aceptado3 == 1 && strcmp(tabla[id].nombre3, nombre)==0)
			{
				if (tabla[id].aceptado4 == 1)
				{
					sprintf(listado, "%s%s, %s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre2, tabla[id].nombre4);
				...
				{
					sprintf(listado, "%s%s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre2);
				}
				else if (tabla[id].aceptado1 == 1)
				{
					sprintf(listado, "%s%s, ", listado, tabla[id].nombre1);
			...
			else if (tabla[id].aceptado4 == 1 && strcmp(tabla[id].nombre4, nombre)==0)
			{
				if (tabla[id].aceptado3 == 1)
				{
					sprintf(listado, "%s%s, %s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre2, tabla[id].nombre3);
				}
				else if (tabla[id].aceptado2 == 1)
				{
					sprintf(listado, "%s%s, %s, ", listado, tabla[id].nombre1, tabla[id].nombre2);
...
int RecordPersonal(TablaPartidas tabla, char nombre[20])	//Devuelve la puntuación máxima conseguida or ese jugador
{
	int res=0;
	for (int i=0;i<100;i++){
		if (tabla[i].aceptado1 == 1 && strcmp(tabla[i].nombre1, nombre)==0)
		{
			if (res<tabla[i].puntos1)
				res=tabla[i].puntos1;
		}
		else if (tabla[i].aceptado1 == 1 && strcmp(tabla[i].nombre2, nombre)==0)
		{
			if (res<tabla[i].puntos2)
				res=tabla[i].puntos2;
		}
		else if (tabla[i].aceptado1 == 1 && strcmp(tabla[i].nombre3, nombre)==0)
		{
			if (res<tabla[i].puntos3)
				res=tabla[i].puntos3;
		}
		else if (tabla[i].aceptado1 == 1 && strcmp(tabla[i].nombre4, nombre)==0)
		{
			if (res<tabla[i].puntos4)
				res=tabla[i].puntos4;
		}
	}
	return res;
...
void InsertDatosFinal(MYSQL *conn, int id_partida, int n_jugadores, char tiempo[20]) //Realiza las consultas a la base de datos de las peticiones
{
	MYSQL_RES *resultado;
	MYSQL_ROW row;
	int err;
	...
	sprintf (consulta,"INSERT INTO Partidas VALUES (%d, %d, '%s');", id_partida, n_jugadores, tiempo);
	printf("consulta: %s\n", consulta);
	err=mysql_query (conn, consulta);
	
	for (int cont = 0; cont < n_jugadores; cont++) 
	{
		if (cont == 0) 
		{
			sprintf (consulta1,"INSERT INTO Participaciones VALUES (%d, %d, %d, '%s');", tabla[id_partida].id1, id_partida, tabla[id_partida].puntos1, tabla[id_partida].color1);
			err=mysql_query (conn, consulta1);
			printf("consulta1: %s\n", consulta1);
		}
		else if (cont == 1) 
		{
			sprintf (consulta1,"INSERT INTO Participaciones VALUES (%d, %d, %d, '%s');", tabla[id_partida].id2, id_partida, tabla[id_partida].puntos2, tabla[id_partida].color2);
			err=mysql_query (conn, consulta1);
			printf("consulta1: %s\n", consulta1);
		}
		else if (cont == 2) 
		{
			sprintf (consulta1,"INSERT INTO Participaciones VALUES (%d, %d, %d, '%s');", tabla[id_partida].id3, id_partida, tabla[id_partida].puntos3, tabla[id_partida].color3);
			err=mysql_query (conn, consulta1);
			printf("consulta1: %s\n", consulta1);
		}
		else 
		{
			sprintf (consulta1,"INSERT INTO Participaciones VALUES (%d, %d, %d, '%s');", tabla[id_partida].id4, id_partida, tabla[id_partida].puntos4, tabla[id_partida].color4);
			err=mysql_query (conn, consulta1);
			printf("consulta1: %s\n", consulta1);
		}
	}
...
void consultarBBDD(MYSQL *conn, int codigo, char respuestaconsulta[512],char nombre[20], char contrasena[50]) //Realiza las consultas a la base de datos de las peticiones
{
	...
	if(codigo == 4||codigo == 10)//Registro del ususario y darse de baja
	{
	...
	else if (codigo == 100){ //Iniciar sesión
		sprintf (consulta, "SELECT Jugadores.id FROM Jugadores WHERE Jugadores.nombre = '%s'",nombre);
	}
	...
	else if (codigo!=4&&codigo!=10&&codigo!=100){
	...
	if(codigo == 10)
	{
		...
		if (row == NULL)
		{
			...
			resultado = mysql_store_result(conn);
			int numRows = mysql_num_rows(resultado);
			if ( numRows == 0 )  
			{
				printf("11/Usuario no existe o combinacion USER-PWD incorrecta\n");
				strcpy (respuestaconsulta, "11/Usuario no existe o combinacion USER-PWD incorrecta");
			}
			else 
			{
				row = mysql_fetch_row (resultado);
	...
	if (codigo == 100)
	{
		resultado = mysql_store_result(conn);
		row = mysql_fetch_row (resultado);
		strcpy (respuestaconsulta, row[0]);
	}
}
...
void *AtenderUsuario (void *socket)
{
	...
	while (terminar==0){
		...
		if (codigo==0){ //Desconexión del servidor
		...
		else if (codigo ==3){	//Récord del jugador
			int res = RecordPersonal(tabla, nombre);
			if (res==0)
			{
				strcpy(respuesta, "3/No has jugado ninguna partida");
				printf("%s", respuesta);				
				write (sock_conn,respuesta, strlen(respuesta));
			}
			else{
				sprintf(respuesta, "3/Tu récord es de %d puntos", res);
				printf("%s", respuesta);				
				write (sock_conn,respuesta, strlen(respuesta));
			}
		}
			...
			printf("Los jugadores son: %s, %s, %s, %s\n", tabla[id].nombre1, tabla[id].nombre2, tabla[id].nombre3, tabla[id].nombre4);
		}
			...
			for (int cont = 0; cont < n_jug; cont++) {
				p = strtok( NULL, "/");
				strcpy(p_nom,p);
				p = strtok( NULL, "/");
				p_puntos = atoi(p);
				...
				consultarBBDD(conn, 100, respuestaconsulta, p_nom, "");
				if (cont == 0) 
				{
					tabla[ident].id1 = atoi(respuestaconsulta);
					tabla[ident].puntos1 = p_puntos;
					strcpy(tabla[ident].color1,p_color);
				}
				else if (cont == 1) 
				{
					tabla[ident].id2 = atoi(respuestaconsulta);
					tabla[ident].puntos2 = p_puntos;
					strcpy(tabla[ident].color2,p_color);
				}
				else if (cont == 2) 
				{
					tabla[ident].id3 = atoi(respuestaconsulta);
					tabla[ident].puntos3 = p_puntos;
					strcpy(tabla[ident].color3,p_color);
				}
				else 
				{
					tabla[ident].id4 = atoi(respuestaconsulta);
					tabla[ident].puntos4 = p_puntos;
					strcpy(tabla[ident].color4,p_color);
				}
				
				printf("puntos: %d\n, id: %d\n, color: %s\n", p_puntos, atoi(respuestaconsulta), p_color);
				
			}
			InsertDatosFinal(conn, ident, n_jug, tiempo);
			Inicializar(tabla,ident);
